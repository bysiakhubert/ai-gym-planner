...
export async function cleanupTestData() {
  try {
    ...
    const { error: sessionsError } = await supabaseAdmin.from("training_sessions").delete().eq("user_id", testUserId);
    ...
    const { error: plansError } = await supabaseAdmin.from("plans").delete().eq("user_id", testUserId);
    ...
    const { error: auditError } = await supabaseAdmin.from("audit_events").delete().eq("user_id", testUserId);
